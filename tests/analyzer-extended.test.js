...
describe('LoopAnalyzer Extended', () => {
    ...
    for (let beat = 0; beat < duration / beatInterval; beat++) {
      ...
      for (let i = 0; i < 1000; i++) {
        if (startSample + i < data.length) {
          data[startSample + i] = Math.sin(2 * Math.PI * 100 * (i / sampleRate)) * Math.exp(-i / 100);
        }
      }
    }
  ...
  test('detectTempo handles 240 BPM', () => {
    ...
    for (let beat = 0; beat < duration / beatInterval; beat++) {
      ...
      for (let i = 0; i < 500; i++) {
        if (startSample + i < data.length) {
          data[startSample + i] = Math.sin(2 * Math.PI * 100 * (i / sampleRate)) * Math.exp(-i / 50);
        }
      }
    }
  ...
  test('detectSlices returns valid slices', () => {
    ...
    for (let hit = 0; hit < 4; hit++) {
      ...
      for (let i = 0; i < 2000; i++) {
        if (startSample + i < data.length) {
          data[startSample + i] = Math.sin(2 * Math.PI * 200 * (i / sampleRate)) * Math.exp(-i / 200);
        }
      }
    }
