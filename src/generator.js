...
export class LoopGenerator {
  ...
  async generateLead(config) {
    return await this.generateMelodic(config); // Simplified
  }
  ...
  async generatePercussion(config) {
    return await this.generateRhythmic(config); // Simplified
  }
  ...
  generateMelody(key, scale, length) {
    ...
    for (let i = 0; i < length; i++) {
      if (this.determinism.nextFloat() < 0.7) { // 70% chance of note
  ...
  getScaleNotes(key, scale) {
    const keyMap = { 'C': 60, 'C#': 61, 'D': 62, 'D#': 63, 'E': 64, 'F': 65, 
